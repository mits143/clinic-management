...
class CancelledFragment : BaseFragment<FragmentActiveBinding>(), AppointmentAdapter.OnClick {
    ...
    private fun setActiveAppointmentData(list: ArrayList<AppointmentData>) {
        val adapter = AppointmentAdapter(arrayListOf(), this, false)
        binding.rvAppointment.adapter = adapter
        adapter.addData(list)
    }

    private fun setObserver() {
        viewModel.fetchCancelAppointmentData("Bearer " + prefs.accessToken, "1")
