...
) : RecyclerView.Adapter<RadiologyAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        with(holder) {
            with(dataList[position]) {
                if (this.radiology_lab_result!!.contains(".pdf"))
                    Glide.with(itemView.context).asBitmap().load(R.drawable.pdf)
                        .into(binding.img)
                else
                    Glide.with(itemView.context).asBitmap().load(this.radiology_lab_result)
                        .into(binding.img)
